...
@router.get("/{product_id}/orders/{order_id}/details/{order_detail_id}/product/category/products/{product_id}")
async def read_product_order_detail_product_category_product(product_id: int, order_id: int, order_detail_id: int):
    db = SessionLocal()
    product = db.query(Product).join(Category).join(OrderDetail).filter(OrderDetail.product_id == product_id, OrderDetail.order_id == order_id, OrderDetail.id == order_detail_id).first()
    db.close()
    if product is None:
        raise HTTPException(status_code=404, detail="Product not found")
    return product

@router.get("/{product_id}/orders/{order_id}/details/{order_detail_id}/product/category/products/{product_id}/orders")
async def read_product_order_detail_product_category_product_orders(product_id: int, order_id: int, order_detail_id: int):
    db = SessionLocal()
    orders = db.query(Order).join(OrderDetail).join(Product).join(Category).filter(OrderDetail.product_id == product_id, OrderDetail.order_id == order_id, OrderDetail.id == order_detail_id).all()
    db.close()
    return orders

@router.get("/{product_id}/orders/{order_id}/details/{order_detail_id}/product/category/products/{product_id}/orders/{order_id}")
async def read_product_order_detail_product_category_product_order(product_id: int, order_id: int, order_detail_id: int):
    db = SessionLocal()
    order = db.query(Order).join(OrderDetail).join(Product).join(Category).filter(OrderDetail.product_id == product_id, OrderDetail.order_id == order_id, OrderDetail.id == order_detail_id).first()
    db.close()
    if order is None:
        raise HTTPException(status_code=404, detail="Order not found")
    return order

@router.get("/{product_id}/orders/{order_id}/details/{order_detail_id}/product/category/products/{product_id}/orders/{order_id}/user")
async def read_product_order_detail_product_category_product_order_user(product_id: int, order_id: int, order_detail_id: int):
    db = SessionLocal()
    user = db.query(User).join(Order).join(OrderDetail).join(Product).join(Category).filter(OrderDetail.product_id == product_id, OrderDetail.order_id == order_id, OrderDetail.id == order_detail_id).first()
    db.close()
    if user is None:
        raise HTTPException(status_code=404, detail="User not found")
    return user
